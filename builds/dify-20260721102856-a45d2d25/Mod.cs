...
namespace ModProject
{
    ...
    public class LiveStudioClient
    {
        ...
        public void Stop()
        {
            // Cancel token first, then force-abort the websocket.
            // Never call CloseAsync(...).Wait(...) from the GTA main thread.
        ...
        private async Task ReceiveLoopAsync(CancellationToken ct)
        {
            ...
            while (!ct.IsCancellationRequested)
            {
                ...
                {
                    // Force-abort avoids the internal sync CloseAsync path used by Dispose.
                    try { _ws?.Abort(); } catch { }
    ...
    public class Mod : Script
    {
        ...
        {
            // 取消 WebSocket（cancel + Abort，不阻塞主线程）
            _client?.Stop();
