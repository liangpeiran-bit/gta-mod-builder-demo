...
Assert-FileExists "templates/live-studio/ModProject/GeneratedGameplay.cs"
...
Assert-FileExists "templates/live-studio/template-contract.json"
Assert-FileExists "templates/live-studio/scripts/validate-generated-gameplay.ps1"
Assert-FileExists "templates/live-studio/ContractTests/ContractTests.csproj"
Assert-FileExists "templates/live-studio/ContractTests/Program.cs"
Assert-DirectoryExists "templates/live-studio/ContractTests/fixtures"
...
Assert-Contains ".github/workflows/build-mod.yml" "template_contract_version:" "workflow_dispatch must accept the template contract version."
Assert-Contains ".github/workflows/build-mod.yml" "templates/live-studio" "Workflow must copy the live-studio template."
Assert-Contains ".github/workflows/build-mod.yml" "ModProject/GeneratedGameplay.cs" "Workflow must write generated gameplay without replacing fixed Mod.cs."
...
Assert-Contains "templates/live-studio/ModProject/Mod.cs" "public\s+partial\s+class\s+Mod" "Fixed Mod.cs must own the partial Script lifecycle shell."
Assert-Contains "templates/live-studio/ModProject/Mod.cs" "MainThreadDispatcher\.Enqueue\(\(\)\s*=>\s*DispatchEventOnMainThread\(evt\)\)" "Fixed runtime must marshal parsed events to the GTA main thread."
Assert-Contains "templates/live-studio/ModProject/Mod.cs" "TriggerGiftOnce" "Fixed runtime must own one-shot gift matching and combo termination."
Assert-Contains "templates/live-studio/ModProject/Mod.cs" "GiftTerminalFallbackDelay" "Fixed runtime must provide a missing-terminal gift fallback."
Assert-Contains "templates/live-studio/ModProject/GeneratedGameplay.cs" "public\s+partial\s+class\s+Mod" "Generated gameplay template must use a partial Mod class."
Assert-NotContains "templates/live-studio/ModProject/GeneratedGameplay.cs" "ClientWebSocket|JavaScriptSerializer" "Generated gameplay must not contain protocol implementation."
Assert-NotContains "templates/live-studio/ModProject/GeneratedGameplay.cs" "EnqueueGameplay" "Generated hooks already run on the GTA main thread and must not enqueue again."
Assert-Contains "templates/live-studio/ModProject/LiveStudio/LiveStudioParser.cs" 'GetBool\(body, "repeatEnd"\)' "Parser must normalize repeatEnd values."
...
Assert-Contains "templates/live-studio/ModProject/LiveStudio/LiveStudioClient.cs" "subscription acknowledged" "LiveStudioClient must distinguish socket connection from subscription acknowledgement."
...
Assert-Contains "templates/live-studio/template-contract.json" '"contract_version":\s*"live-studio-template-3"' "Template contract must be version 3."
...
Assert-Contains "pages/status.html" "GeneratedGameplay.cs" "Status page should label the generated gameplay source correctly."
...
Assert-Contains "docs/dify-integration.md" "GeneratedGameplay.cs" "Dify integration docs must describe the generated gameplay boundary."
...
Assert-Contains "templates/live-studio/references/template-contract.md" "GeneratedGameplay.cs" "Template contract must document the generated gameplay file."
Assert-Contains "templates/live-studio/references/template-contract.md" "TriggerGiftOnce" "Template contract must document the fixed one-shot gift helper."
Assert-Contains "templates/live-studio/references/template-contract.md" "all generated hooks run on the GTA" "Template contract must document fixed main-thread hook dispatch."
...
Assert-Contains "templates/live-studio/references/template-contract.md" "three-argument.*World.CreateVehicle" "Template contract must document CreateVehicle overload constraints."
Assert-Contains "docs/dify-knowledge/live-studio-template/template-contract.md" "TriggerGiftOnce" "Dify template knowledge must document the fixed gift helper."
